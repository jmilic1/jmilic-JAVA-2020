 ...
 * @author Jura Milić
 *
 * @param <T> type of data that is stored within Collection
 */
public class ArrayIndexedCollection<T> implements List<T> {
	 ...
	 * @author Jura Milić
	 * 
	 * @param <E> type of objects the class will return
	 */
	private static class ArrayGetter<E> implements ElementsGetter<E> {
		/**
		 * The Collection whose elements are being returned.
		 */
		private ArrayIndexedCollection<E> coll;
		 ...
		 */
		public ArrayGetter(ArrayIndexedCollection<E> collection) {
			coll = collection;
	...
	public ArrayIndexedCollection(Collection<? extends T> other, int initialCapacity) {
		...
		this.elements = (T[]) new Object[Math.max(other.size(), initialCapacity)];
		size = 0;
	...
	public void add(T value) {
		...
		if (size >= elements.length) {
			ArrayIndexedCollection<T> arrayColl = new ArrayIndexedCollection<T>(this, size * 2);
